...
if [ $? -ne 0 ]; then
    ...
    sed -i -e "s~%%RSET%%~${replset}~g" -e "s~%%FAILURE%%~${failmsg}~g" /var/scripts/backup/email/failure-notification.html
...
EOF
    ...
    sed -i -e "s~%%RSET%%~${replset}~g" -e "s~%%ADDED%%~${DEST}.bz2~g" -e "s~%%REMOVED%%~${remdir}.bz2~g" /var/scripts/backup/email/success-notification.html
    
    # Send email - !! DO NOT indent Email headers & EOF below !!
    cat <<'EOF' - /var/scripts/backup/email/success-notification.html | /usr/sbin/sendmail -i -t -f "brian.benton@bbc.co.uk"
To: brian.benton@bbc.co.uk
Reply-To: mongod@$hostname
...
# Upload to AWS S3
~/.local/bin/aws s3 sync $WKDIR s3://vivo-test-dst/mongo-backups/int
